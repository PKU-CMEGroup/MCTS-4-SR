...
def build_summary(by_dataset: pd.DataFrame) -> pd.DataFrame:
    """Algorithm-level summary using median per-dataset ranks.

    The rank columns use ``median`` aggregation, consistent with the
    coordinates used in ``save_pareto_rank``.
    """
    return (
        ...
        .agg(
            ...
            median_r2_rank=("r2_test_rank", "median"),
            median_model_size=("model_size", "median"),
            median_model_size_rank=("model_size_rank", "median"),
            median_training_time_s=("training time (s)", "median"),
            median_training_time_rank=("training_time_rank", "median"),
        )
        .sort_values(["median_r2_rank", "median_model_size_rank"])
    )
